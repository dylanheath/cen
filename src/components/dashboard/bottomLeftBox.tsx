...
export default function BottomLeftBox() {
  ...
  useEffect(() => {
    ...
    const fetchTransactions = async () => {
      ...
      if (address) {
      const getTransactionsReceived = await axios.get(`${api.url}/transactions/getreceived${address}`, { timeout: 2000 })
        .then((response) => {
	  ...
	  const ReceivedPush = TransactionsReceived.map((transaction: any) => transactionsBundle.push(transaction));
        ...
        const getTransactionSent = await axios.get(`${api.url}/transactions/getsent${address}`, { timeout: 2000 })
          .then((response) => {
